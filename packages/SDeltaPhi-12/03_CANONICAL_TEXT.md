...
- **Axiom 1 — Trace necessity.** If irreversible traces do not re-enter selection, catastrophic repeats occur.
- **Axiom 2 — Trace overload.** If all traces re-enter, feasible path space collapses (fixation/paralysis).
- **Axiom 3 — Regulation.** Persistence requires regulated effective memory:

```text
M_eff_t := C(H_M(Δϕ_0:t))
```

- **Axiom 4 — Freedom coupling.** Forgetting is the minimal condition for freedom under irreversibility, but excessive forgetting increases repeat risk.
...
## 4. Mechanism: Trace Regulation Loop (TRL)
...
Memory injects constraints; forgetting compresses/prunes constraints to prevent path-space collapse. The system persists only when both remain active.

## 5. Pathology map

| Regime | Condition | Observable behavior | Structural risk |
|---|---|---|---|
| Drift / Repeat | Memory too weak (M↓) | Repeated failure loops; no stable constraints | Catastrophic recurrence |
| Fixation / Paralysis | Forgetting too weak (F↓) | Overconstraint; feasible path-space collapse | Stagnation / self-locking |
...
RollbackCost may be approximated by explanation cost, repair/recovery cost, social/legal cost, and time-to-restore. A minimal selection heuristic is to re-enter traces with high recurrence risk and high irreversibility magnitude, while discarding low-impact traces via compression.

## 8. Position in the series

SΔϕ-12 derives directly from SΔϕ-11 and reframes persistence as regulated trace re-entry. It also relies on SΔϕ-09 and SΔϕ-10.
